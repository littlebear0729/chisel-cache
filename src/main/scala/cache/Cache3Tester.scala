...
class Cache3Tester(dut: Cache3) extends PeekPokeTester(dut) {
  ...
  scala.Predef.printf(
    s"[Tester] numAccesses: ${numAccesses}, numHits: ${numHits}, hitRatio: ${numHits.toDouble / numAccesses}, numCycles: ${numCycles}\n"
  );
}

object Cache3Tester extends App {
  chisel3.iotesters.Driver(() => new Cache3(), "treadle") { dut =>
    new Cache3Tester(dut)
  }
}
